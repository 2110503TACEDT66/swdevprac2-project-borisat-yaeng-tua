...
import { MassageShopItem } from "@/interfaces";
import CustomButton from "@/components/button";

export default function Bookings({ params }: { params: { hid: string } }) {
    const [bookingDate, setBookingDate] = useState<Dayjs | null>(null);
    const { data: session } = useSession();
    const [massageShopDetail, setMassgeShopDetail] = useState<MassageShopItem | null>(null);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const res = await fetch(`http://localhost:5000/api/v1/massagespas/${params.hid}/appointments`, {
                method: 'POST',
                headers: {
    ...
    return (
        <main className="w-[100%] flex flex-col items-center space-y-4">
            <form onSubmit={handleSubmit} className="fixed translate-x-[-50%] translate-y-[-50%] top-[30%] left-[50%] mt-[42.5px] z-10 p-[15px] rounded-xl backdrop-blur bg-gray-500 bg-opacity-30 flex flex-col justify-center">
                ...
                <Link href="/">
                <CustomButton containerStyles="block rounded-md bg-sky-600 hover:bg-indigo-600 px-3 py-2 text-white shadow-sm"
                    title="Book Massage"
